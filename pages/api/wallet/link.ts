...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  let userWallet = await supabaseServerClient
    .from('user_wallet')
    .select('*')
    .match({ wallet: address, signed_nonce: true })
  ...
  userWallet = await supabaseServerClient
    .from('user_wallet')
    .select('*')
    .match({ wallet: address, user_id: user.user.id })
  if (userWallet.data.length === 0) {
